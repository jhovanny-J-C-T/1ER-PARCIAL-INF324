...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['monto'])) {
    ...
    // Realizar la operación de retiro en la cuenta
    $sql_retiro = "UPDATE cuentabancaria SET saldo = saldo - $monto WHERE idcuenta = '$cuenta_id'";
    if (mysqli_query($conexion, $sql_retiro)) {
        // Registrar la transacción en la tabla transaccion
        $sql_transaccion = "INSERT INTO transaccion (cuenta_origen_id, tipo_transaccion, monto) VALUES ('$cuenta_id', 'retiro', $monto)";
        mysqli_query($conexion, $sql_transaccion);

        header('Location: cliente.php');
    } else {
        echo "Error al realizar el retiro: " . mysqli_error($conexion);
    }
}
